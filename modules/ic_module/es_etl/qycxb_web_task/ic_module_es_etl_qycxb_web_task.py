...
def main():
    ...
    es_status_success = 1
    select_company = " SELECT id, company_name, parsed_content FROM qycxb_web_company where parse_status = %s and es_status !=%s limit %s "
...
def handle_single_item(item, **kwargs):
    company_id, company_name, parsed_content = item['id'], item['company_name'].strip(), item['parsed_content']
    ...
    DbHelper.execute("UPDATE qycxb_web_company set es_status = %s WHERE id = %s ", (kwargs['es_status_success'], company_id))
