    ...
    while True:
        ...
        sql = '''
            select b.id, b.update_time, b.src_gsgs, b.src_qygs, b.src_bmgs, b.src_sfgs from (
                select id from gs_fujian_company where status = %s and parse_status = %s  limit %s
            ) as a left join (select id, update_time, src_gsgs, src_qygs, src_bmgs, src_sfgs from gs_fujian_company) as b on a.id = b.id
        '''
        ...
        for item in items:
            ...
            gsgs, qygs, bmqs, sfgs = item['src_gsgs'], item['src_qygs'], item['src_bmgs'], item['src_sfgs']
            try:
                ...
                DbHelper.execute("update gs_fujian_company set parse_status = %s, data_table_name= %s where id = %s",
                ...
                DbHelper.execute("update gs_fujian_company set parse_status = %s where id = %s", [2, company_id])
                continue
